...
export function JobDescriptionPanel({ onClose }) {
    ...
    <div className="space-y-4">
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <div className="sm:col-span-2">
          ...
          <Input value={skills} onChange={(e) => setSkills(e.target.value)} placeholder="e.g. Python, FastAPI, PostgreSQL, AWS" />
        </div>
      </div>

      <Button onClick={onGenerate} disabled={generate.isPending}>
        <Sparkles className="size-4" />
        {generate.isPending ? 'Generating…' : hasResult ? 'Regenerate' : 'Generate description'}
      </Button>
      ...
      {hasResult && (
        <div className="space-y-4 border-t pt-4">
          ...
          <div>
            ...
            <Input value={skillsResult} onChange={(e) => setSkillsResult(e.target.value)} />
          </div>
          <div className="flex justify-end gap-2">
            <Button variant="outline" onClick={copy}>Copy</Button>
            <Button onClick={createJob}>Create job with this</Button>
          </div>
        </div>
      )}
    </div>
